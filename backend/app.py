# Simple adult-content heuristic (no heavy model): checks common adult keywords/domains
from typing import Optional
ADULT_KEYWORDS = [
    '18+', 'nsfw', 'xxx', 'porn', 'sex', 'adult', 'fetish', 'xvideo', 'xvideos', 'xnxx',
    'redtube', 'pornhub', 'brazzers', 'camgirl', 'onlyfans'
]

def is_adult_content(name_or_url: Optional[str]) -> bool:
    if not name_or_url:
        return False
    s = name_or_url.lower()
    return any(k in s for k in ADULT_KEYWORDS)
...
from backend.models.runtime_infer import predict_image, predict_video, predict_audio
...
async def analyze_auto(
    ...
    - If text present (and no file), treat as text.
    - If file present, infer by extension (image/video/audio).
    - If only URL present, infer type by URL heuristics; use placeholders for media bytes.
    For videos, compute a combined audio+video suspicion by blending scores.
    """
    try:
        ...
        def infer_type_from_name(name: str) -> str:
            n = (name or "").lower()
            if any(n.endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp"]):
                return "image"
            if any(n.endswith(ext) for ext in [".mp4", ".avi", ".mov", ".mkv", ".webm", ".gif"]):
                return "video"
            if any(n.endswith(ext) for ext in [".mp3", ".wav", ".m4a", ".ogg", ".flac"]):
                return "audio"
            if "youtube.com" in n or "youtu.be" in n:  # heuristic
                return "video"
            return "unknown"
        ...
        if text and not file:
            content_type = "text"
            content_repr = text
            fake_result = detect_fake_news(text, lang)
            fake_score = fake_result.get("score", 0.0)
        elif file is not None:
            inferred = infer_type_from_name(file.filename)
            if inferred == "image":
                file_bytes = await file.read()
                fake_score = calculate_image_fake_score(file_bytes, file.filename)
                content_type = "image"
                content_repr = f"Image file: {file.filename}"
            elif inferred == "audio":
                file_bytes = await file.read()
                fake_score = calculate_audio_fake_score(file_bytes, file.filename)
                content_type = "audio"
                content_repr = f"Audio file: {file.filename}"
            else:
                # default to video
                file_bytes = await file.read()
                v_score = calculate_video_fake_score(file_bytes, file.filename)
                # Approximate audio suspicion from the same bytes (placeholder)
                a_score = calculate_audio_fake_score(file_bytes[: min(50000, len(file_bytes))], file.filename)
                fake_score = min(1.0, max(0.0, 0.6 * v_score + 0.4 * a_score))
                content_type = "video"
                content_repr = f"Video file: {file.filename}"
        elif source_url:
            inferred = infer_type_from_name(source_url)
            content_type = inferred if inferred != "unknown" else "text"
            content_repr = f"URL: {source_url}"
            if content_type == "video":
                # Placeholder separate component scores for URL-only videos
                v_score = 0.5
                a_score = 0.5
                fake_score = min(1.0, max(0.0, 0.6 * v_score + 0.4 * a_score))
            elif content_type == "text":
                fake_score = detect_fake_news(source_url, lang).get("score", 0.0)
            else:
                fake_score = 0.5  # neutral for other media via URL
        else:
            raise HTTPException(status_code=400, detail="Provide text, a file, or a source_url")

        # Build AnalysisRequest and run enhanced features
        request = AnalysisRequest(
            content=content_repr if content_type != "text" else (text or content_repr),
        ...
        analysis.adult_content = is_adult_content(source_url) or is_adult_content(file.filename if file else None)

        # Add explanation note and metrics for combined video analysis
        if content_type == "video":
            if isinstance(analysis.explanation, dict):
                analysis.explanation.setdefault("summary", "Video analyzed across visual and audio cues")
                analysis.explanation.setdefault("detailed_analysis", {}).setdefault(
                    "technical_indicators",
                    [
                        "Visual deepfake heuristics (artifacts, motion inconsistencies)",
                        "Audio suspicion heuristic blended with visual score",
                    ],
                )
                # Try to expose separate component scores when available
                try:
                    v = locals().get("v_score", None)
                    a = locals().get("a_score", None)
                except Exception:
                    v, a = None, None
                analysis.explanation.setdefault("metrics", {})
                if v is not None:
                    analysis.explanation["metrics"]["visual_score"] = v
                if a is not None:
                    analysis.explanation["metrics"]["audio_score"] = a
...
async def analyze_text(text: str = Form(...), lang: str = Form("en"), source_url: str = Form(None)):
    ...
    analysis = await comprehensive_analysis(request, BackgroundTasks())
    analysis.adult_content = is_adult_content(source_url) or is_adult_content(text)
    return analysis

@app.post("/analyze_image/")
async def analyze_image(file: UploadFile | None = File(None), source_url: str = Form(None)):
    """Enhanced image analysis with all features"""
    try:
        # Handle file or URL-only submissions
        if file is not None:
            file_content = await file.read()
            # Try model inference by saving to a temp path
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file.filename or "image.jpg")[1] or ".jpg")
            try:
                tmp.write(file_content)
                tmp.flush(); tmp.close()
                model_prob = predict_image(tmp.name)
            finally:
                try: os.remove(tmp.name)
                except Exception: pass
            if model_prob is not None:
                fake_score = float(model_prob)
            else:
                fake_score = calculate_image_fake_score(file_content, file.filename)
            content = f"Image file: {file.filename}"
        else:
            # URL-only analysis path
            fake_score = 0.5
            content = f"Image URL: {source_url}" if source_url else "Image content"
        request = AnalysisRequest(
            content=content,
            content_type="image",
            source_url=source_url
...
async def analyze_video(file: UploadFile | None = File(None), source_url: str = Form(None)):
    """Enhanced video analysis with all features"""
    try:
        # Handle file or URL-only submissions
        if file is not None:
            file_content = await file.read()
            # Save to temp and try model inference (visual+audio)
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file.filename or "video.mp4")[1] or ".mp4")
            try:
                tmp.write(file_content)
                tmp.flush(); tmp.close()
                v_prob, a_prob, fused = predict_video(tmp.name)
            finally:
                try: os.remove(tmp.name)
                except Exception: pass
            if fused is not None:
                v_score = v_prob if v_prob is not None else None
                a_score = a_prob if a_prob is not None else None
                fake_score = float(fused)
            else:
                # fallback heuristic
                v_score = calculate_video_fake_score(file_content, file.filename)
                a_score = calculate_audio_fake_score(file_content[: min(50000, len(file_content))], file.filename)
                fake_score = min(1.0, max(0.0, 0.6 * v_score + 0.4 * a_score))
            content = f"Video file: {file.filename}"
        else:
            fake_score = 0.5
            content = f"Video URL: {source_url}" if source_url else "Video content"
        request = AnalysisRequest(
            content=content,
            content_type="video",
            source_url=source_url
...
async def analyze_audio(file: UploadFile | None = File(None), source_url: str = Form(None)):
    """Enhanced audio analysis with all features"""
    try:
        if file is not None:
            file_content = await file.read()
            # Save to temp and try model inference
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file.filename or "audio.wav")[1] or ".wav")
            try:
                tmp.write(file_content)
                tmp.flush(); tmp.close()
                model_prob = predict_audio(tmp.name)
            finally:
                try: os.remove(tmp.name)
                except Exception: pass
            if model_prob is not None:
            ...
            fake_score = 0.5
            content = f"Audio URL: {source_url}" if source_url else "Audio content"
        request = AnalysisRequest(
            content=content,
            content_type="audio",
            source_url=source_url

...
def calculate_image_fake_score(file_content: bytes, filename: str) -> float:
    ...
    try:
        ...
        if filename:
            suspicious_keywords = ['fake', 'deepfake', 'generated', 'ai', 'synthetic', 'fake_', 'deep_']
...
def calculate_video_fake_score(file_content: bytes, filename: str) -> float:
    ...
    try:
        ...
        # Check file size
        if len(file_content) < 5000:  # Less than 5KB
            fake_score += 0.4
        ...
        if filename:
            suspicious_keywords = ['fake', 'deepfake', 'generated', 'ai', 'synthetic']
            if any(keyword in filename.lower() for keyword in suspicious_keywords):
                fake_score += 0.7
